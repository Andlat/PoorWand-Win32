...
//TODO Modal to ask if certain to delete element or section
//TODO When SAVE is clicked, verify if PHP session is still valid (in case someone finds out this js file's name and loads it to mess with the website)
...
/*
  ...
  $.post("isadmin.php", {session: document.cookie}, function(r)){
    if(r){
      $("body").append('<button id="add_section">Add Section</button>');
    }
  }
  */
});
    ...
    showModal('', function(rsp){
      if(zone.files.length > 0){
        ...
        if(rsp){//Add the newly added file to the section
          ...
          let width = Number.parseInt($('#media_width').val());
          let height = Number.parseInt($('#media_height').val());

          if(gtype == type.image) elmnt = '<img id="' + id + '" src="media/' + name + '"/>';
          else if(gtype == type.video) elmnt = '<video id="' + id + '" controls><source src="media/' + name + '" type="video/mp4"/></video>';
          $(window.selected).append(elmnt);

          let jelmnt = $('#'+id);

          if(!isNaN(width)) jelmnt.css('width',width);
          if(!isNaN(height)) jelmnt.css('height',height);
...
function setSave(){
  ...
  function getHtml(){
    ...
    doc.replace(/style\=\".*\"/g, '');//remove the inline style tags
    doc.replace(/id\=\"ce[0-9]\"/g, '');//strip the ids used for the editor
    doc.replace(/contenteditable\=\"(true|false)\"/g, '');//replace all the contenteditable props
    console.log(doc);
  ...
  $('body').append('<button id="add_section" class="adminButton">Add Section</button>\
                  ...
                  \
                    <div id="confirmation_modal">\
                      ...
                      <div id="media_input">\
                        <input type="number" id="media_width" class="media_input" placeholder="width"/>\
                        <span>x</span>\
                        <input type="number" id="media_height" class="media_input" placeholder="height"/>\
                      </div>\
                    ...
                    <script id="dropzone_script">setDropZoneOption();</script>\
                    ');
}
